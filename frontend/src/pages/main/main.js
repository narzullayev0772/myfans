import './main.css';
import {Form,Button,} from "react-bootstrap"
import { useState} from 'react';
import axios from 'axios';
import { Link } from 'react-router-dom';
const Main = () => {
 const defaultsrc = 'data:image/png;base64,'


    const [deadline,setDeadline] = useState('');
    const [izoh,setIzoh] = useState('');
    
    
    var dataFile = new FormData();
    const handleFile = (e)=>{
        e.preventDefault();
        const file = e.target.files[0];
        dataFile.append('name',file.name);
        dataFile.append('file',file);

        fetch('/files',{
            method:'POST',
            body:dataFile
        }).then(res=>res.json())
        .then(succ=>console.log(succ))
        .catch(err=>console.log(err))
    }

    function fileUpload() {
        const obj = {
            izoh:izoh,
            deadline:deadline
        }
        

        axios.post('/text',obj)
        .then(res=>{
            console.log(res);
        })
        .catch(err=>console.log(err))       
    }



    return ( 
        <div className="main">
            <div className="main-nav" >
                <Link to="/">
                <div className="main-logo">
                    MyFan
                </div>
                </Link>
               <div>
              <Link to="/settings" className="account-settings">
                  <img src={defaultsrc} className="account-image" alt="" />
              <h4>Account</h4>
              </Link>
               </div>
            </div>

            <div className="main-body">
                <div className="main-left">
<Form> 
    <Form.Group controlId="formFile" className="mb-3">
    <Form.Control type="file" onChange={handleFile} required accept=".pdf, .docx, .doc, .ppt, .pptx, .xls, .xlsx, .zip, .rar" />
  </Form.Group>
  <Form.Group className="mb-3" controlId="exampleForm.ControlTextarea1">
    <Form.Control style={{resize:'none'}} value={izoh} onChange={e=>{setIzoh(e.target.value)}} as="textarea" rows={10} placeholder="❕ Izoh : " required/>
  </Form.Group>
  <Form.Control value={deadline} onChange={(e)=>{setDeadline(e.target.value)}} className="mb-3" type="text" placeholder="🕛 Deadline Vaqti: (1-yanvar) " required/>
  <Button onClick={fileUpload} variant="primary" type="submit">
  📤 Yubormoq
  </Button>
</Form>
                </div>
                <div className="main-right">
                    <h3 className="link-header"> Tayyor Filelar: </h3>
                        <ul className="download">
                            <li><a href="link">File name</a></li>
                            <li><a href="link">File name</a></li>
                            <li><a href="link">File name</a></li>
                            <li><a href="link">File name</a></li>

                        </ul>
                    
                </div>
            </div>
        </div>
     );
}
 
export default Main;